...
def analisar_historico_creditos(json_data: dict, data_consulta=None):
    ...
    for credito in creditos:
        ...
        # A regra original olhava todas as datas, causando falsos positivos.
        # Restringimos a busca aos campos relevantes: Competência (Início Período) e Previsão (Início Validade)
        datas_relevantes = f"{dt_inicio_periodo} {dt_inicio_validade}"
        if any(alvo in datas_relevantes for alvo in meses_alvo):
